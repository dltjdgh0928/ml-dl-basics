...
xy_train = train_datagen.flow_from_directory('d:/study_data/_data/brain/train/', target_size=(100, 100), batch_size=5000, class_mode='binary', 
                                  color_mode='grayscale', shuffle=True)

# categorical : onehot 벡터로 변환

# Found 160 images belonging to 2 classes. ( 전체 데이터 쓸려면 batch_size 160 이상 )

xy_test = test_datagen.flow_from_directory('d:/study_data/_data/brain/test/', target_size=(100, 100), batch_size=1500, class_mode='binary', 
                                  color_mode='grayscale', shuffle=True)
...
print(xy_train[0][0].shape)     # (160, 200, 200, 1) : grayscale
print(xy_test[0][0].shape)      # (120, 200, 200, 1) : grayscale
