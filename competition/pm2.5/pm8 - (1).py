    ...
    for k in range(64):
        for i in range(120):
            if np.isnan(test_pm[j, 120*k+i, 1]) and i<84:
                test_pm[j, 120*k+i, 1] = model1.predict(np.concatenate([test_pm[j, 120*k+i-11:120*k+i-1, 1:], test_pm_aws[j, 120*k+i-11:120*k+i-1, :]], axis=1).reshape(-1,timesteps,7).astype(np.float32))
            elif i>=84:
                test_pm[j, 120*k+204-i-1, 1] = model2.predict(np.flip(np.concatenate([test_pm[j, 120*k+204-i:120*k+204-i+10, 1:], test_pm_aws[j, 120*k+204-i:120*k+204-i+10, :]], axis=1), axis=0).reshape(-1,timesteps,7).astype(np.float32))
            print(f'model 변환 진행중{j}의 {k}의 {i}번')
        a=np.append(a, test_pm[j, 120*k+48:120*k+120, 1])
