...
train_aws['month'] = train_aws['일시'].str[:2].astype('int8')
train_aws['hour'] = train_aws['일시'].str[6:8].astype('int8')
train_aws = train_aws.drop(['연도', '일시'], axis=1)

test_aws['month'] = test_aws['일시'].str[:2].astype('int8')
test_aws['hour'] = test_aws['일시'].str[6:8].astype('int8')
...
print(test_aws)
# pm
# 측정소 PM2.5 month hour


# aws
# 지역 기온 풍향 풍속 강수량 습도 month hour
    ...
    for k in range(64):
        for i in range(120):
            if np.isnan(test_pm[j, 120*k+i, 1]) and i<84:
                test_pm[j, 120*k+i, 1] = model1.predict(np.concatenate([test_pm[j, 120*k+i-1-timesteps:120*k+i-1, 1:], test_pm_aws[j, 120*k+i-1-timesteps:120*k+i-1, :]], axis=1).reshape(-1, timesteps, 7).astype(np.float32))
            elif i>=84:
                test_pm[j, 120*k+204-i-1, 1] = model2.predict(np.flip(np.concatenate([test_pm[j, 120*k+204-i:120*k+204-i+10+timesteps, 1:], test_pm_aws[j, 120*k+204-i:120*k+204-i+10+timesteps, :]], axis=1), axis=0).reshape(-1, timesteps, 7).astype(np.float32))
...
submission['PM2.5'] = submission['PM2.5'].apply(lambda x: np.array(eval(x)))

flatten = lambda x: x.flatten()[0]

submission['PM2.5'] = submission['PM2.5'].apply(flatten)

submission.to_csv('C:/AIA/AIA-study/_data/pm2.5/Aiur_submission_4.csv')
