...
# class counter:
#     def __init__(self,org_count:int)->None:
...
#             return True
#         else:
#             self.count=self.org_count
#             return False

# # if counter(test_pm[j, 120*k+48:120*k+120, 1]):
    ...
    for k in range(64):
        for i in range(120):
            if np.isnan(test_pm[j, 120*k+i, 1]) and i<84:
                test_pm[j, 120*k+i, 1] = model1.predict(np.concatenate([test_pm[j, 120*k+i-1-timesteps:120*k+i-1, :], test_pm_aws[j, 120*k+i-1-timesteps:120*k+i-1, :]], axis=1).reshape(-1, timesteps, x.shape[2]).astype(np.float32))
            elif i>=84:
