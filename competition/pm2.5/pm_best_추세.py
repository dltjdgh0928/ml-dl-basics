...
os.listdir('C:/study/_save/finedust/best/Aiur_Submit_1353')

for i,v in enumerate(os.listdir('C:/study/_save/finedust/best/Aiur_Submit_1353')):
    globals()['model{}'.format(i+1)]=load_model(f'C:/study/_save/finedust/best/Aiur_Submit_1353/{v}')


# 5.1 predict
l=[]
filled_pm=np.zeros(120)
for j in range(17):
    for k in range(64):
        filled_pm[:48]+=test_pm_onehot[j, 120*k:120*k+48, 0]
        for i in range(72):
        ...
        filled_pm[48:]+=np.array(l)
filled_pm=filled_pm/(17*64)
print('# 5.1 Done')

filled_pm=pd.Series(filled_pm,name='PM2.5')
# filled_pm.to_csv('./Data_for_PM_mean')
import matplotlib.pyplot as plt
plt.plot(range(120),filled_pm.values)
plt.show()
